...
# vending_machine_MM.py - (Windows O/S File) - Python3
...
# Code adapted from base file provided by
# Professors Phil Jarvis & Louis Bertrand 
...
try:
    ...
    servo = Servo(17)                 # servo on GPIO Pin 17
    key1 = Button(5)                  # return button on GPIO Pin 5
...
def log(s):
    """Logs the steps and events as the program goes through the states"""
    if TESTING:
        print(s)
...
class VendingMachine(object):
    #List of products and value in cents
    PRODUCTS = {"suprise ": ("SURPRISE", 5), "pop ": ("Pop", 50), "chips ": ("Chips", 75),
                "choclate ": ("Choclate", 150), "gummies ": ("Gummies", 225)
...
#         # Rotate Servo motor through three complete cylces (Dispense product)
#         count = 0
#         while count < 3:
#             servo.min()
#             sleep(1)
#             servo.mid()        # Commented out for Windows application
#             sleep(1)
#             servo.max()
#             sleep(1)
#             count = count + 1
...
if __name__ == "__main__":
    ...
    print=sg.Print            #Print program responses in GUI
    coin_col = []
    ...
    for item, item_prices in VendingMachine.PRODUCTS.items():
        log(item)
        prices = item_prices[1]
        price = sg.Text(f"¢{prices}", font=("Helvetica", 18))
        button = sg.Button(item, font=("Helvetica", 18))
        row = [button, price]
